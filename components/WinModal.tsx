...
export const WinModal: React.FC<WinModalProps> = ({ timeMs, onPlayAgain, topic, analysis, language }) => {
  ...
  const getShareText = () => {
    // Basic substitution for the message
    const timeStr = formatTime(timeMs);
    let message = t.shareMessage
      .replace('{time}', timeStr)
      .replace('{topic}', topic);
      
    if (analysis) {
       message += ` ${t.boredomScore}: ${analysis.boredomScore}% 😴`;
    }
    
    return `${message} #MeetingBingoAI`;
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/80 backdrop-blur-sm animate-in fade-in duration-300">
      <div className="bg-white rounded-3xl shadow-2xl p-6 max-w-md w-full text-center transform animate-in zoom-in-95 duration-300 max-h-[90vh] overflow-y-auto">
        ...
        <div className="mb-6">
          ...
          <h2 className="text-3xl font-bold text-slate-800">{t.winTitle}</h2>
        </div>
        ...
        <div className="bg-gradient-to-br from-indigo-50 to-purple-50 rounded-2xl p-6 mb-6 border border-indigo-100">
           {!analysis ? (
             <div className="flex flex-col items-center justify-center py-4 gap-3">
               ...
               <p className="text-sm text-indigo-600 font-medium animate-pulse">{t.thinking}</p>
             </div>
           ) : (
             <div className="animate-in slide-in-from-bottom-4 fade-in duration-500">
               <div className="flex justify-between items-center mb-4 pb-4 border-b border-indigo-100">
                 <div className="text-left">
                   <p className="text-xs uppercase tracking-wider text-indigo-400 font-bold">{t.time}</p>
                   <p className="text-xl font-mono font-bold text-slate-700">{formatTime(timeMs)}</p>
                 </div>
                 <div className="text-right">
                   <p className="text-xs uppercase tracking-wider text-indigo-400 font-bold">{t.boredomScore}</p>
        ...
        <h3 className="text-sm font-semibold text-slate-400 uppercase tracking-widest mb-3">{t.shareTitle}</h3>
